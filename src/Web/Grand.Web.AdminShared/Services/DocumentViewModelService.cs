...
public class DocumentViewModelService : IDocumentViewModelService
{
    ...
    {
        var documents = await _documentService.GetAll(model.SearchName, model.SearchNumber,
            model.SearchEmail, reference: (Reference)model.Reference, objectId: model.ObjectId, status: model.StatusId,
    ...
    public virtual async Task<DocumentModel> PrepareDocumentModel(DocumentModel documentModel, Document document,
                    ...
                    switch (simpleModel.Reference)
                    {
                        case (int)Reference.Order:
                            ...
                            if (order != null)
                            {
                                ...
                                model.OutstandAmount =
                                    order.PaymentStatusId == PaymentStatus.Paid ? 0 : order.OrderTotal;
                                model.CurrencyCode = order.CustomerCurrencyCode;
                                model.Name = string.Format(_translationService.GetResource("Order.Document"),
                                    model.Number);
                        ...
                        case (int)Reference.Shipment:
                            ...
                            if (shipment != null)
                            {
                                ...
                                model.Name = string.Format(_translationService.GetResource("Shipment.Document"),
                        ...
                        case (int)Reference.MerchandiseReturn:
                            var merchandisereturn =
                                await _merchandiseReturnService.GetMerchandiseReturnById(simpleModel.ObjectId);
                            if (merchandisereturn != null)
                            {
                                ...
                                model.Name =
                                    string.Format(_translationService.GetResource("MerchandiseReturns.Document"),
                                        merchandisereturn.ReturnNumber);
